...
/*
If an array has n elements and all elements are inserted in binary search tree then the tree has space complexity of O(n) and the array also has the space complexity of O(n)
The space complexity of searching a node in a BST would be O(n) with 'n' being the depth of the tree(number of nodes present in a tree) since at any point of time maximum number of stack frames that could be present in memory is 'n'.
*/
