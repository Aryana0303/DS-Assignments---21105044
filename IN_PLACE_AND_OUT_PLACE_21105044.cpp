/*                         
ANS 1:-
Space complexity "In-Place" sorting algorithm lies between O(1) and O(logn) both included.
An in-place algorithm is an algorithm that needs a small extra space and produces an output in the same memory that
contains the data by transforming the input ‘in-place’ whereas an 
"Out-Place" sorting algorithm is an algorithm whose space complexity is greater than O(logn).
Sorting algorithms may require some extra space for comparison and temporary storage of few data elements. 
These algorithms do not require any extra space and sorting is said to happen in-place, or for example, within the array itself. This is called in-place sorting. Bubble sort is an example of in-place sorting.
...
apply in algorithm whereas out-place techniques are easy to apply but they 
increase the space complexity of the algorithm.
...
extra array.
IN-PLACE algorithm is used in Bubble sort, Selection Sort, Insertion Sort, Heapsort.
